export default {
    ...
    getSubgroupAnchor(group, subgroup, subgroupIndex = 0) {
        ...
        return `bvs-settings-subgroup-${group.id}-${subgroupIndex + 1}-${subgroupSlug || 'section'}`;
    },
    getSettingsToolsResetAnchor() {
        return 'bvs-settings-tools-reset-defaults';
    },
    ...
    getSearchResults(structure, settings, normalizedQuery) {
        ...
        for ( let group of structure ) {
            ...
            const groupResult = {
                ...
                targetId: `bvs-settings-group-${group.id}`,
    ...
    groupMatchesSearch(group, normalizedQuery) {
        ...
        if (
            this.matchesSearch(group.name, normalizedQuery)
    ...
    subgroupMatchesSearch(subgroup, normalizedQuery) {
        ...
        if (
            ( subgroup.name && this.matchesSearch(subgroup.name, normalizedQuery) )
